...
The `home-routes` handler in the `pet-eligentis.routes.home` namespace
...
The `home-page` function will in turn call the `pet-eligentis.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `pet-eligentis.handler` namespace:
...
Request middleware functions are located under the `pet-eligentis.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `pet-eligentis.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `pet-eligentis.core` in some other namespace.
